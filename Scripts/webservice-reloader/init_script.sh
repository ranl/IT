...
[ -f "/etc/sysconfig/$DAEMON" ] && source /etc/sysconfig/$DAEMON

# Settings to put in /etc/sysconfig/$DAEMON
#URL=
#GREP=
#CMD=
#TIMEOUT=
...
function _start()
{
	...
	then
		...
		webservice-reloader.py -u "$URL" -g "$GREP" -c "$CMD" -t "$TIMEOUT" -r "$RETRIES" -s "$SLEEP" &> /dev/null &
    echo $! > $PIDFILE
    sleep 2
